...
use indicatif::ProgressBar;
    ...
    fn execute_second_action() {
        print!("Enter cell size: ");
        io::stdout().flush().unwrap();
        let mut cell_size_decode = String::new();
        io::stdin().read_line(&mut cell_size_decode).unwrap();
        let image_path = "null_block_qr_grid.png";
        let decoded_url = decode_snaking_qr_grid(image_path,cell_size_decode.trim().parse().unwrap());
